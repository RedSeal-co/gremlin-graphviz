...
var factory = module.exports = function (gremlinGraph, opts) {
  var graphvizGraph;
  // Allow the arguments to be promises by using BluePromise.join
  return BluePromise
    .join(gremlinGraph, opts, function (g, opts) {
      gremlinGraph = g;
      assert(gremlinGraph);
    ...
    .then(function () {
      var vertexTraversal = gremlinGraph.traversal().V();
      return TP.forEach(vertexTraversal, function (vertex) { addVertex(graphvizGraph, vertex); });
    })
    .then(function () {
      var edgeTraversal = gremlinGraph.traversal().E();
...
var addEdge = function (graphvizGraph, gremlinEdge) {
  ...
  var inV = gremlinEdge.inVertex();
  assert(inV);
  var inId = vertexId(inV);
  var outV = gremlinEdge.outVertex();
  assert(outV);
